...
export default function AdminPromotionsPage() {
  ...
  const activas = promos.filter(p => p.estado === 'ACTIVO').length;
  const combos = promos.filter(p => p.tipo === 'COMBO').length;
  const usosTotales = promos.reduce((acc, p) => acc + p.usosActuales, 0);
  ...
  return (
    <div className="w-full flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: 'Promociones Activas', value: String(activas), trend: `${promos.length} en total`, color: 'text-rose-700 dark:text-rose-300', bg: 'bg-rose-100/70 dark:bg-rose-500/20', border: 'border-rose-200 dark:border-rose-500/40', icon: TrendingDown },
          { label: 'Combos Oferta', value: String(combos), trend: 'Ofertas tipo combo', color: 'text-purple-700 dark:text-purple-300', bg: 'bg-purple-100/70 dark:bg-purple-500/20', border: 'border-purple-200 dark:border-purple-500/40', icon: Gift },
          { label: 'Cupones Redimidos', value: String(usosTotales), trend: 'Usos totales registrados', color: 'text-emerald-700 dark:text-emerald-300', bg: 'bg-emerald-100/70 dark:bg-emerald-500/20', border: 'border-emerald-200 dark:border-emerald-500/40', icon: Percent },
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center gap-3 text-slate-400 py-20">
          ...
          <p className="text-sm font-black uppercase tracking-widest">Cargando promociones...</p>
        </div>
      ) : error ? (
        <div className="flex flex-col items-center justify-center gap-4 py-20">
          <AlertTriangle className="w-12 h-12 text-rose-500" />
          <p className="text-sm font-bold text-slate-500 dark:text-slate-400">{error}</p>
          <button onClick={load} className="px-6 py-2 bg-rose-50 text-rose-600 font-bold rounded-xl border border-rose-100">Cargar de nuevo</button>
        </div>
      ) : filteredPromos.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 text-slate-300 py-20">
          <Tag className="w-16 h-16 opacity-20" />
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No hay promociones todavía</p>
          <button onClick={() => navigate('/admin/promociones/nuevo')} className="px-6 py-2 bg-rose-600 text-white font-bold rounded-xl">Crear la primera</button>
        </div>
      ) : (
                    ...
                    {openMenu === promo.id && (
                      <div className="absolute right-0 top-6 w-40 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-20 overflow-hidden">
                        <button onClick={() => { setOpenMenu(null); navigate(`/admin/promociones/editar/${promo.id}`); }}
                          className="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">
                          Editar
                        </button>
              ...
              <div className="px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between rounded-b-2xl mt-auto">
                <div className="flex flex-col gap-1">
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                    {promo.usosActuales}{promo.maxUsos ? ` / ${promo.maxUsos}` : ''} canjeos / usos
                  </span>
                  {promo.fechaInicio && (
